...
const Cuaca = {
    async render() {
        return `<div class="cuaca-container"></div>`;
    },

    async afterRender(prov = 'sumatera-utara', city = 'medan') {
        const cuaca = await WeatherEarthquakeSource.getCuaca(prov, city);
        console.log(cuaca);
        ...
        WeatherEarthquakeSource.getProvinsi();
        ...
        document.querySelector('#provinsi').addEventListener('change', ()=> {
            ...
            WeatherEarthquakeSource.getCityFromProv(formatText);
        })
